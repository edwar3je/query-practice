...
SELECT SUM(reviews) as total_number_of_reviews FROM analytics;
-- Find all the categories that have more than 300 apps in them. --
SELECT category, COUNT(app_name) AS number_of_apps FROM analytics GROUP BY category HAVING COUNT(app_name) > 300;
...
SELECT app_name, reviews, min_installs, min_installs/reviews AS proportion_install_by_reviews FROM analytics WHERE min_installs > 100000 ORDER BY min_installs/reviews DESC LIMIT 1;